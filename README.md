...
Методы класса:
`render(items: HTMLElement[]): HTMLElement` - принимает HTML для рендера на главную страницу

#### galeryCardView
Класс отвечает за создание карточек для главной странице сайта

`constructor(private events: IEvents)` - принимает действия пользователя

Методы класса:
`Render()` - создает карточку товара для галереи
